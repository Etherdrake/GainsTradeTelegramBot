...
type PairMinLevResponse struct {
	PairMinLev int64  `json:"pair_min_lev"`
	PairIndex  int64  `json:"pair_index"`
	Chain      string `json:"chain"`
}

func GetPairMinLev(pairIndex uint64, chain uint64) (uint64, error) {
	url := os.Getenv("HOOT_CORE_API_LOCAL")
	if url == "" {
		return 0, errors.New("environment variable HOOT_CORE_API_LOCAL is not set")
	}
	url += "getpairminleverage"

	requestData := GetPairMinLeveragePayload{
		Index: pairIndex,
		Chain: chain,
	}

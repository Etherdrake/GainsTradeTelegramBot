...
func (rdbHoot *HootRedisClient) UpdateTrade(trade types.TradeItemMongo) error {
	...
	// Use JSON.SET to update the serialized trade in Redis
	...
	if statusCmd.Err() != nil {
		log.Println("Error updating trade in Redis:", statusCmd.Err())
...
func (rdbHoot *HootRedisClient) UpdateOrder(trade types.TradeItemMongo) error {
	// Create a context with a timeout
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second) // Set timeout duration as needed
	defer cancel()                                                          // Ensure the cancel function is called to release resources
	...
	statusCmd := rdbHoot.JSONSet(ctx, key, ".", string(tradeJSON))
	if statusCmd.Err() != nil {
		log.Println("Error updating order in Redis:", statusCmd.Err())
		return statusCmd.Err()
	}

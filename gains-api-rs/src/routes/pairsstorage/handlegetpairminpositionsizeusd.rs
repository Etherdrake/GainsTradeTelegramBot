...
use crate::gains::pairsstorage::getpairminpossizeusd::get_pair_min_pos_size_usd;
use crate::routes::pairsstorage::payloads::{GetPairMinPosSizeUsd};

pub async fn handle_get_pair_min_position_size_usd(
    Json(payload): Json<GetPairMinPosSizeUsd>,
) -> impl IntoResponse {
    match get_pair_min_pos_size_usd(
        payload.index,
        payload.chain,
    ).await {
        Ok(orders) => (

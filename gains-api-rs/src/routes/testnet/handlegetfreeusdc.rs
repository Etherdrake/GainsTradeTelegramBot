...
use crate::gains::testnet::getfreeusdc::get_free_usdc;

pub async fn handle_get_free_usdc(
    Json(payload): Json<crate::routes::testnet::payloads::GetFreeDaiPayload>,
) -> impl IntoResponse {
    match get_free_usdc(payload.guid, payload.chain).await {
        Ok(orders) => (
            StatusCode::OK,
            Json(Ok(orders)),
        ),
        Err(e) => {
            ...
            error!("Error in handle_get_free_usdc: {}", error_message);
            (

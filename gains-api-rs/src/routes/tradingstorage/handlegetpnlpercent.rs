...
use crate::gains::tradingstorage::read::getpnlpercent::get_pnl_percent;
use crate::gains::tradingstorage::tradingstoragetypes::trading_storage::{PendingOrderStruct};

pub async fn handle_get_pnl_percent(
    Json(payload): Json<crate::routes::tradingstorage::payloads::GetPnlPercentPayload>,
) -> (StatusCode, Json<Result<I256, ErrorMessageResponse>>) {
    match get_pnl_percent(
        payload.open_price,
        ...
        Ok(pnl_percent) => (StatusCode::OK, Json(Ok(pnl_percent))),
        Err(e) => {

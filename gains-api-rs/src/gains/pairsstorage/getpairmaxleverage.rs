...
pub async fn get_pair_max_leverage(
    index: u64,
    chain: u64
    ...
    let contract = pairMaxLeverage::new(contract_address, Arc::new(client.clone()));

    let index_conv: U256 = U256::from(index);
    let query = contract.pair_max_leverage(index_conv).await?;
